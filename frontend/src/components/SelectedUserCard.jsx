...
const SelectedUserCard = ({ user }) => {
  ...
  if (newUser && remoteAdvice?.advice?.length === 0) {
    getRemoteAdvice().then((data) =>
      setRemoteAdvice({ advice: data, cached: false })
    );
  }
              ...
              <Grid.Column width={10}>
                {!newUser && values && (
                  <>
                    <Card fluid>
                      <Image
                        src={`https://robohash.org/${values.id}.png`}
                      ></Image>
                    </Card>
                  </>
                )}
                ...
                {values && (
                  <>
                    <Popup
                      ...
                      trigger={
                        <Card fluid>
                          <Image
                            size="huge"
                            src={images[backend.active]}
                          ></Image>
                          <Popup
                            position="right center"
                            open={remoteAdvice?.cached}
                            openOnTriggerMouseEnter={false}
                            trigger={
                              <Button
                                disabled={remoteAdvice?.cached || newUser}
                                primary
                                onClick={() => {
                                  getRemoteAdvice().then((data) => {
                          ...
                          >
                            Disabled until{" "}
                            <b>External API's request cache expires</b> (2
                            seconds)
                          </Popup>
                        </Card>
                      }
                    >
                      {values?.advice
                        ? values.advice
                        : "Create a new user for my advice"}
                    </Popup>
                    <Header style={{ marginBottom: "0.2em", marginTop: "0" }}>
                      Update Advice
                    </Header>
                    <Label>{adviceResponseTime} ms</Label>
                  </>
                )}
              </Grid.Column>
            </Grid>
